class Game {
  ...
  start() {
    ...
    runner3.addImage("blast", blastImage);
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        if (index === player.index) {
          ...
          this.handleRunnerACollisionWithRunnerB(index);
  ...
  handleFuel(index) {
    // Adding fuel
    runners[index - 1].overlap(fuels, function(collector, collected) {
      player.fuel = 185;
  ...
  handlePowerCoins(index) {
    runners[index - 1].overlap(powerCoins, function(collector, collected) {
      player.score += 21;
      player.update();
      //collected is the sprite in the group collectibles that triggered
      //the event
  ...
  handleRunnerACollisionWithRunnerB(index) {
    if (index === 1) {
      if (runners[index - 1].collide(runners[1]) || runners[index - 1].collide(runners[2])) {
        if (this.leftKeyActive) {
          player.positionX += 100;
        } else {
          player.positionX -= 100;
    ...
    if (index === 2) {
      if (runners[index - 1].collide(runners[0]) || runners[index - 1].collide(runners[2])) {
        if (this.leftKeyActive) {
          player.positionX += 100;
        } else {
          player.positionX -= 100;
    ...
    if (index === 3) {
      if (runners[index - 1].collide(runners[0]) || runners[index - 1].collide(runners[1])) {
        if (this.leftKeyActive) {
          player.positionX += 100;
        } else {
          player.positionX -= 100;
